...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size, to_tensor=False, device=None):
        ...
        if to_tensor:
            return tuple(map(lambda x: torch.tensor(x).to(device), experience_tuples)) + (idxes,)  # , weights)
        else:
    ...
    def sample_obs(self, batch_size, to_tensor=False, device=None):
        ...
        idxes = self.tree.sample(batch_size)
        if to_tensor:
            return torch.tensor(self.obs[idxes]).to(device)
        else:
    ...
    def get_all_data(self, max_samples=None, to_tensor=False, device=None):
        ...
        if to_tensor:
            return tuple(map(lambda x: torch.tensor(x).to(device), tuples))
        else:
            return tuples
